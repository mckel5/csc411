...
pub struct Array2IterRowMajor<'a, T> {
    row: usize,
    column: usize,
    array2: &'a Array2<T>,
}
...
pub struct Array2IterColumnMajor<'a, T> {
    row: usize,
    column: usize,
    array2: &'a Array2<T>,
}
...
impl<T: Clone> Array2<T> {
    // Constructors
    // INVARIANT: - Any instance of `Array2` with type `T` will have a concrete width and height,
    // each greater than zero. Each element will be a value of type `T`.
    ...
    // Accessor
    // INVARIANT: Requesting the value at coordinates `(x, y)` will produce the value at the
    // `x`th row and `y`th column of the `Array2`. `Array2`s are zero-indexed.

    /// Retrieve the element at position `(row, col)`. Returns `None` if the element does not exist.
    ...
    // Iterators
    // INVARIANT: Row-major and column-major iterators may be requested from an `Array2`
    // regardless of the type of `Vec` used to initialize the array (or the underlying
    // implementation).
...
impl<'a, T: Clone> Iterator for Array2IterRowMajor<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let value = self.array2.get(self.row, self.column).cloned();
